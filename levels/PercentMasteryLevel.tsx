...
const PercentMasteryLevel: React.FC<LevelComponentProps> = ({ onComplete, onExit, partialProgress, onSavePartialProgress }) => {
  const [taskIndex, setTaskIndex] = useState(() => partialProgress?.taskIndex || 0);
  const [isInstructionOpen, setIsInstructionOpen] = useState(false);
  const [isGlossaryOpen, setIsGlossaryOpen] = useState(false);
  const totalTasks = 7; // Placeholder count
...
export default PercentMasteryLevel;
