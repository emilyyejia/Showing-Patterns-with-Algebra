...
const PatternLevel1: React.FC<LevelComponentProps> = ({ onComplete, onExit, partialProgress, onSavePartialProgress }) => {
  ...
  useEffect(() => {
    return () => {
      if (!isCompletedRef.current && onSavePartialProgress) {
        onSavePartialProgress({ step, attempts });
      }
    };
  }, [step, attempts]); // Removed onSavePartialProgress to prevent infinite loops
  ...
  const validateT1 = () => {
    ...
    if (s1 && s2 && s3) {
      ...
      let hintMsg = "Try again! Check the pattern carefully.";
      if (t1Term3 !== '11' && t1Term3 !== '') hintMsg = "";
  ...
  const validateTable = () => {
    ...
    if (isTableIncomplete) { handleIncorrect("Complete all table cells first."); return; }
    ...
    if (s5 && s6 && s7) {
      ...
      handleIncorrect("Check the subtraction! 28 - 3 = 25.");
    }
  };
  ...
  const validateT2 = () => {
    ...
    if (sB && s9) {
      handleCorrect(3);
        ...
        if (t2Behavior !== 'decreases' && t2Behavior !== '') handleIncorrect("Try again! Check the pattern carefully");
        else if (v9 !== 13 && t2Term9 !== '') handleIncorrect("Try again! Check the pattern carefully.");
        else handleIncorrect("Try again! Check the pattern carefully.");
    }
  };
  ...
  const validateT3 = () => {
    ...
    if (sB && sV && sT) {
      ...
      if (!sB || !sV) {
        handleIncorrect("Look at the first two dots. At Term 1, the value is 3. At Term 2, it’s 6. What is the pattern?");
      } else {
  ...
  const validateT4 = () => {
    ...
    else handleIncorrect("Look at the number of tiles in each Figure: 2, 4, 8, 16… What is the pattern?");
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-full p-6 text-white font-sans max-w-5xl mx-auto relative">
      ...
      {step === 2 && (
        <p className="text-center text-white mb-6 text-2xl font-bold leading-relaxed animate-fade-in">
          {isTableComplete ? 'Fill in the boxes.' : 'Complete the table.'}
        </p>
      )}
      ...
      <div className="w-full max-w-3xl bg-gray-800 rounded-3xl p-8 shadow-2xl border border-gray-700 relative">
          ...
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-[100] animate-fade-in p-4">
              <div className="bg-emerald-600 p-8 rounded-3xl text-center shadow-2xl max-w-md border-4 border-emerald-400">
                  ...
                  <div className="flex flex-col gap-3">
                      <button onClick={() => { setShowT5Popup(false); setT5Input(''); setT5FinalResult(''); setValidationStatus({}); setFeedback(null); }} className="bg-white text-emerald-600 font-black py-3 px-8 rounded-xl hover:bg-emerald-50 transition-colors active:scale-95">Try Another Number</button>
                      <button 
                        onClick={() => { 
                          let stars = 3;
        ...
        {step === 1 && (
          <div className="animate-fade-in text-center pt-4">
            <div className="bg-gray-900 p-10 rounded-2xl mb-10 flex justify-center gap-4 text-6xl font-mono text-indigo-300 tracking-widest shadow-inner border border-gray-700">3, 7, 11, 15, 19, 23, ...</div>
            <div className="space-y-8 text-2xl text-gray-300 bg-gray-800/50 p-10 rounded-xl border border-gray-700">
              <div className="flex flex-wrap items-center justify-center gap-4">
                <span>The pattern</span>
                <select className={`bg-gray-900 border-2 px-4 py-3 rounded-lg text-xl focus:outline-none transition-colors ${getStatusColor('t1Behavior')}`} value={t1Behavior} onChange={(e) => { setT1Behavior(e.target.value); setFeedback(null); clearStatus('t1Behavior'); }}><option value="">-- select --</option><option value="increases">increases</option><option value="decreases">decreases</option></select>
                <span>by</span>
                <select className={`bg-gray-900 border-2 px-4 py-3 rounded-lg text-xl focus:outline-none transition-colors ${getStatusColor('t1Value')}`} value={t1Value} onChange={(e) => { setT1Value(e.target.value); setFeedback(null); clearStatus('t1Value'); }}><option value="">--</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select>
                <span>each time!</span>
              </div>
              <div className="flex flex-wrap items-center justify-center gap-4">
                <span>What's the value for the 3rd term?</span>
                <input type="number" className={`w-28 bg-gray-900 border-2 p-3 text-center text-xl rounded-lg focus:outline-none transition-colors ${getStatusColor('t1Term3')}`} value={t1Term3} onChange={(e) => { setT1Term3(e.target.value); setFeedback(null); clearStatus('t1Term3'); }} />
              </div>
              {feedback && (
                <div className={`mt-4 px-8 py-4 rounded-xl font-bold text-xl shadow-lg animate-fade-in text-center ${
                  feedback.type === 'correct' ? 'bg-emerald-500 text-white' : 'bg-rose-600 text-white border-2 border-rose-400'
                }`}>
                  {feedback.message || (feedback.type === 'correct' ? '🌟 Correct!' : 'Try again! Check the pattern carefully.')}
                </div>
              )}
              <button onClick={validateT1} className="mt-4 bg-sky-600 hover:bg-sky-500 px-12 py-5 rounded-xl text-xl font-black transition-all shadow-lg hover:scale-105 uppercase tracking-wider">Check</button>
            </div>
          </div>
        )}
        {step === 2 && (
              ...
              <div className="flex flex-col gap-4">
                <table className="w-full md:w-64 border-collapse bg-gray-900 rounded-xl overflow-hidden shadow-xl border border-gray-600">
                  ...
                  <tbody>
                    ...
                    {[5,6,7].map(n => <tr key={n} className="bg-indigo-900/10"><td className="p-3 border border-gray-600 text-center font-mono">{n}</td><td className="p-3 border border-gray-600 text-center"><input type="number" className={`w-16 bg-gray-900 text-center border-2 rounded p-1 focus:outline-none transition-colors ${getStatusColor(`t2Value${n}`)}`} value={t2Values[n as 5|6|7]} onChange={e => { setT2Values({...t2Values, [n]: e.target.value}); setFeedback(null); clearStatus(`t2Value${n}`); }} disabled={isTableComplete} /></td></tr>)}
                  </tbody>
                </table>
                {!isTableComplete && (
                  <button onClick={validateTable} className="w-full bg-sky-600 hover:bg-sky-500 py-4 rounded-xl font-black transition-all shadow-lg uppercase tracking-wider">Check</button>
                )}
              </div>
              {isTableComplete && (
                <div className="flex-1 space-y-6 text-xl bg-gray-800/50 p-6 rounded-xl border border-gray-700 self-stretch flex flex-col transition-all duration-500 animate-fade-in">
        ...
        {step === 3 && (
          <div className="animate-fade-in text-center pt-4">
            ...
            <div className="space-y-6 text-xl bg-gray-900/50 p-8 rounded-xl border border-gray-700">
              <div className="flex flex-wrap items-center justify-center gap-3"><span>The pattern</span><select className={`bg-gray-900 border-2 rounded-lg p-2 text-lg transition-colors ${getStatusColor('t3Behavior')}`} value={t3Behavior} onChange={(e) => { setT3Behavior(e.target.value); setFeedback(null); clearStatus('t3Behavior'); }}><option value="">-- select --</option><option value="increases">increases</option><option value="decreases">decreases</option></select><span>by</span><select className={`bg-gray-900 border-2 rounded-lg p-2 text-lg transition-colors ${getStatusColor('t3Value')}`} value={t3Value} onChange={(e) => { setT3Value(e.target.value); setFeedback(null); clearStatus('t3Value'); }}><option value="">--</option>{[2,3,4,5].map(v => <option key={v} value={v}>{v}</option>)}</select><span>each time!</span></div>
              <div className="flex items-center justify-center gap-3"><span>What's the 5th term value?</span><input type="number" className={`w-24 bg-gray-900 border-2 rounded-lg p-2 text-center text-lg transition-colors ${getStatusColor('t3Term5')}`} value={t3Term5} onChange={(e) => { setT3Term5(e.target.value); setFeedback(null); clearStatus('t3Term5'); }} /></div>
              {feedback && (
        ...
        {step === 4 && (
          <div className="animate-fade-in text-center pt-4">
             <div className="flex flex-wrap justify-center gap-8 mb-10 overflow-x-auto pb-4 px-2">
               {[2, 4, 8, 16].map((count, i) => (
                 <div key={i} className="flex flex-col items-center gap-3 shrink-0 bg-gray-900/50 p-6 rounded-2xl border border-gray-700 shadow-xl"><div className="grid grid-cols-4 gap-2 w-32 h-32">{Array.from({length: count}).map((_, j) => <div key={j} className="w-7 h-7 bg-emerald-500 rounded-sm shadow-[0_0_5px_rgba(16,185,129,0.5)]" />)}</div><span className="text-lg font-bold text-gray-400 mt-2">Figure {i+1}</span></div>
               ))}
             </div>
             <div className="space-y-8 text-2xl bg-gray-800/50 p-10 rounded-2xl border border-gray-700 shadow-2xl">
                <div className="flex flex-wrap items-center justify-center gap-4"><span>The pattern</span><select className={`bg-gray-900 border-2 rounded-lg p-3 text-xl transition-colors ${getStatusColor('t4Behavior', 'border-emerald-500')}`} value={t4Behavior} onChange={(e) => { setT4Behavior(e.target.value); setFeedback(null); clearStatus('t4Behavior'); }}><option value="">-- select --</option><option value="halves">halves</option><option value="doubles">doubles</option><option value="triples">triples</option></select><span>each time!</span></div>
                <div className="flex items-center justify-center gap-4"><span>What's the value of Figure 5?</span><input type="number" className={`w-32 bg-gray-900 border-2 rounded-lg p-3 text-center text-xl transition-colors ${getStatusColor('t4Term5', 'border-emerald-500')}`} value={t4Term5} onChange={(e) => { setT4Term5(e.target.value); setFeedback(null); clearStatus('t4Term5'); }} /></div>
                {feedback && (
                    <div className={`px-6 py-2 rounded-xl font-bold shadow-lg animate-fade-in text-center ${
                    feedback.type === 'correct' ? 'bg-emerald-500 text-white' : 'bg-rose-600 text-white border-2 border-rose-400'
                    }`}>
        ...
        {step === 5 && (
          <div className="animate-fade-in text-center pt-4">
            <div className="mb-10">
              <h2 className="text-5xl font-black text-sky-400 mb-2">The Algebraic Magician</h2>
            </div>

            <div className="bg-gray-800 rounded-3xl p-10 border-2 border-gray-600 shadow-[0_20px_50px_rgba(0,0,0,0.5)] mb-8 text-left space-y-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none"><span className="text-8xl">✨</span></div>
              <div className="flex justify-between items-center bg-gray-900/50 p-6 rounded-2xl border border-gray-600">
                <span className="text-sky-300 font-bold text-2xl">1. Pick any number (your variable 'x'):</span>
                <input type="number" className="w-36 bg-gray-900 border-2 border-sky-400 rounded-xl p-4 text-center text-white text-2xl font-bold shadow-inner" value={t5Input} onChange={e => { setT5Input(e.target.value); setT5FinalResult(''); setFeedback(null); }} placeholder="..." />
              </div>
              
              {t5Input && (
                <div className="space-y-5 animate-fade-in text-gray-200 font-bold text-xl">
                  <div className="flex items-center justify-between bg-gray-700/30 p-4 rounded-lg">
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-xs uppercase text-sky-400 font-black tracking-widest">Algebra Example</span>
                      <span className="text-sky-300 font-mono text-lg">3x</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between bg-gray-700/30 p-4 rounded-lg">
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-xs uppercase text-sky-400 font-black tracking-widest">Algebra Example</span>
                      <span className="text-sky-300 font-mono text-lg">3x + 12</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between bg-gray-700/30 p-4 rounded-lg">
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-xs uppercase text-sky-400 font-black tracking-widest">Algebra Example</span>
                      <span className="text-sky-300 font-mono text-lg">x + 4</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between bg-gray-700/30 p-4 rounded-lg">
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-xs uppercase text-sky-400 font-black tracking-widest">Algebra Example</span>
                      <span className="text-sky-300 font-mono text-lg">(x + 4) - x</span>
                    </div>
                  </div>
            ...
            {validationStatus.t5FinalResult === 'correct' && (
              <div className="animate-fade-in-up bg-gray-900/80 p-10 rounded-2xl border border-emerald-500/30 mt-6">
                <h3 className="text-emerald-400 font-black uppercase tracking-widest mb-4 text-2xl">Behind the Magic:</h3>
